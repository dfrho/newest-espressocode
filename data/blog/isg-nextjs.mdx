...
images: ['/static/images/nathan-dumlao-unsplash.webp']
---

![nextjs code](/static/images/nathan-dumlao-unsplash.webp)
...
The concept of generating static pages on-demand at runtime has been around for a while, and other frameworks such as Gatsby and Hugo have also implemented similar functionality. For example, Gatsby offers a feature called "Incremental Builds," which allows you to build and deploy only the pages that have changed since the last build, rather than rebuilding the entire site from scratch. This can save a significant amount of time and resources, especially for larger websites with many pages. Similarly, Hugo offers a feature called "Fast Render Mode," which generates static pages on-demand at runtime for pages that have been updated since the last build. While the specifics of how ISR is implemented may differ between frameworks, the underlying concept is the same: generating static pages on-demand at runtime for improved performance and flexibility.
