...
Then in your Hybrid page, you can specify the rendering mode for each page in the `getServerSideProps` function of the component. Here's an example of how you can use hybrid mode to switch the rendering mode for a specific page:
...
In this example, the `getServerSideProps` function checks whether the page is being requested server-side or client-side. If the page is being requested server-side, it fetches the data and return it as props. If the page is being requested client-side, it returns an empty props object.
