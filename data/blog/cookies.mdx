...
<p>
  Notice in the dev tools images above in the **Storage** section there are multiple listings.
...
<p>
  ...
  cookies. First we'll install the libraries express, dayjs and cookie-parser:
</p>
...
<p>
  ...
  ```http://localhost:3000/```. Open dev tools and click on the **Application** tab, **Storage**
  section and you should see the flags set noted as below:
</p>
...
<p>
1. Avoid storing sensitive information, such as passwords, as cookies are stored in plain text
2. Restrict that cookie to only your application - __path__ argument
3. Don't Share With Sub Domains: Use the __domain__ option
4. Only for Secure Connections: Using the __secure__ option you can tell the browser (or other http clients) to only send the cookie over SSL connections
5. Protect Against XSS Exploits: The __httpOnly__ flag is used to tell the browser that it should not allow javascript to access the contents of the cookie
</p>
